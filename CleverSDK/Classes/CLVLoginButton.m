...
const CGFloat CLVLoginButtonBaseHeight = 45.6;
...
- (void)scale:(CGFloat)scaleFactor {
    ...
    [self setImage:[CLVLoginButton cleverIconWithSize:self.bounds.size] forState:UIControlStateNormal];
}

- (void)scaleWithWidth:(CGFloat)width {
    CGFloat scaleFactor = width / self.frame.size.width;
    [self scale:scaleFactor];
}

- (void)scaleWithHeight:(CGFloat)height {
    CGFloat scaleFactor = height / self.frame.size.height;
    [self scale:scaleFactor];
}
...
+ (UIImage *)cleverIconWithSize:(CGSize)size {
    ...
    CGFloat scaleX = size.width / CLVLoginButtonBaseWidth;
    CGFloat scaleY = size.height / CLVLoginButtonBaseHeight;
    CGAffineTransform scaleTransform = CGAffineTransformMakeScale(scaleX, scaleY);
    cleverC = CGPathCreateCopyByTransformingPath(cleverC, &scaleTransform);
    CGAffineTransform moveTransform = CGAffineTransformMakeTranslation(10 * scaleX, 10 * scaleY);
...
+ (UIImage *)textForButton:(CGSize)size {
    ...
    CGFloat scaleY = size.height / CLVLoginButtonBaseHeight;
    CGAffineTransform scaleTransform = CGAffineTransformMakeScale(scaleX, scaleY);
    cleverText = CGPathCreateCopyByTransformingPath(cleverText, &scaleTransform);
    CGAffineTransform moveTransform = CGAffineTransformMakeTranslation(15 * scaleX, 10 * scaleY);
